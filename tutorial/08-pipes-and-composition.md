...
`|>`, `<|`, `>>`, `<<`는 컴파일러에 내장된 특수 연산자가 아니라, `Prelude/Core.fun`에 일반 함수로 정의되어 있습니다. 예를 들어 `|>`의 정의는 단 한 줄입니다:
...
`#[left 1]`은 fixity 속성으로, 이 연산자가 좌결합이며 우선순위 레벨 1(가장 낮음)임을 선언합니다. 이 메커니즘 덕분에 사용자도 동일한 방식으로 자신만의 연산자에 원하는 우선순위와 결합성을 부여할 수 있습니다. 자세한 내용은 [13장: 사용자 정의 연산자](13-user-defined-operators.md)를 참조하세요.
