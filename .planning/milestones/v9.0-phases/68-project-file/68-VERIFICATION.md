...
|----------|----------|--------|---------|
| `src/FunLang/ProjectFile.fs` | TOML deserialization, FunProjConfig record, parseFunProj/findFunProj/loadFunProj | VERIFIED | 76 lines; 3 `[<CLIMutable>]` POCO types; `TomlSerializer.Deserialize` wired; null-safe section handling; exported functions match plan spec |
| `src/FunLang/FunLang.fsproj` | Tomlyn 2.3.0 PackageReference + ProjectFile.fs compile entry | VERIFIED | `PackageReference Include="Tomlyn" Version="2.3.0"` present; `<Compile Include="ProjectFile.fs" />` registered before Cli.fs |
| `src/FunLang/Cli.fs` | BuildArgs/TestArgs sub-DU types; Build/Test cases on CliArgs | VERIFIED | 47 lines; `BuildArgs` and `TestArgs` DUs with `[<CliPrefix(CliPrefix.None)>]`; `Build` and `Test` cases on `CliArgs` with correct CliPrefix attributes |
| `src/FunLang/Prelude.fs` | `resolvePreludeDir` and `loadPrelude` with `projPrelude: string option` param | VERIFIED | `resolvePreludeDir (explicitPath: string option) (projPrelude: string option)` at line 247; priority chain correct; `loadPrelude` delegates to it |
| `src/FunLang/Program.fs` | build/test dispatch branches before prelude loading; reads config; per-target processing | VERIFIED | Build dispatch at lines 89–134; test dispatch at lines 136–191; both placed before the `else` branch that loads prelude without projPrelude; config.PreludePath passed to loadPrelude |
