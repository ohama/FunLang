...
FunLang는 F# 스타일의 들여쓰기 구문을 사용하는 정적 타입(statically-typed) 함수형 언어로,
...
`not`이 연산자(`!`)가 아닌 함수라는 점이 흥미롭습니다. FunLang에서는 `not`을 다른 고차 함수에 직접 넘길 수 있습니다. 예를 들어 `filter not [true; false; true]`처럼요 -- 이것이 함수형 언어에서 연산자 대신 함수를 선호하는 이유 중 하나입니다.
...
`if`가 표현식이라는 의미는, 변수에 직접 담을 수 있다는 뜻입니다: `let label = if score > 90 then "A" else "B"`. C나 Java에서는 삼항 연산자(`? :`)를 별도로 두는 반면, FunLang에서는 `if-then-else` 자체가 그 역할을 합니다.
