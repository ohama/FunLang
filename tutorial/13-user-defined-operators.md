...
`#[right 6]`으로 선언했으므로 `1 +-> 2 +-> 3`은 `1 +-> (2 +-> 3)` = `1 +-> 5` = `6`으로 계산됩니다. 기본적으로 `+`로 시작하는 연산자는 좌결합이지만, fixity 속성으로 우결합으로 변경했습니다.

### Prelude의 활용

Prelude의 `|>`, `<|`, `>>`, `<<` 연산자도 이 fixity 속성으로 정의되어 있습니다. `Prelude/Core.fun`을 열어보면:
