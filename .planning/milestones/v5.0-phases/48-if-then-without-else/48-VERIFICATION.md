...
|----------|----------|--------|---------|
| `src/FunLang/Parser.fsy` | New `IF Expr THEN SeqExpr` rule desugaring to `If($2, $4, Tuple([], ...), ...)` | VERIFIED | Line 153: rule present immediately after IF-THEN-ELSE rule; 769 lines |
| `src/FunLang/Parser.fs` | Regenerated LALR(1) tables including the new rule | VERIFIED | Line 1080: `If(_2, _4, Tuple([], symSpan parseState 4), ruleSpan parseState 1 4)` |
