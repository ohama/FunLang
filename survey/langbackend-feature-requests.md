...
**Date:** 2026-03-28
**Author:** Generated from source analysis of FunLang, LangBackend, and FunLexYacc
...
- **LangBackend** (`../LangBackend/`) is the AOT compiler (AST -> MLIR -> LLVM -> native binary). It currently has 144 E2E tests and reaches FunLang v5.0 feature parity for simple programs, but is missing several features required by FunLexYacc.
...
| # | Feature | Category | FunLang | LangBackend | FunLexYacc Files | Priority | Effort |
|---|---------|----------|-----------|-------------|------------------|----------|--------|
...
**LangBackend compiler status:** Missing. Explicitly listed in `PROJECT.md` line 96 as "Out of Scope" with note "complexity high." The compiler uses libc `printf` for print/println but has no format-string parser for the FunLang `%d`/`%s` format mini-language.
...
This is the largest gap. FunLexYacc uses .NET BCL types directly (not through FunLang's builtin hashtable/array). LangBackend has no .NET interop — it compiles to native code via LLVM, so .NET types are unavailable.
...
**LangBackend compiler status:** LangBackend has `hashtable_*` builtins, but these are a different type with different API. The .NET `Dictionary<K,V>` has methods like `.TryGetValue()` that return `(bool, V)` tuple-via-out-parameter — not present in FunLang hashtable builtins.
...
These are functions defined in FunLang's Prelude (`.fun` files) or available as builtins. They work in the interpreter but LangBackend needs to either compile the Prelude files or provide native implementations.
...
**Description:** FunLang automatically loads `Prelude/*.fun` files at startup, making Option, Result, List, Core, Array, Hashtable modules available. LangBackend needs to either compile these Prelude files alongside user code or provide native implementations of all Prelude functions.
