...
tech-stack:
  ...
  patterns:
    ...
    - "Queue.Dequeue is a zero-arg method: takes TupleValue [] arg, raises FunLangException on empty queue"
...
decisions:
  ...
  - id: HS-02-hashset-add
    ...
    rationale: ".NET HashSet<T>.Add returns bool (true if new, false if duplicate); directly maps to FunLang BoolValue"
  - id: HS-03-queue-dequeue
    choice: "Queue.Dequeue takes TupleValue [] (zero-arg method pattern); raises FunLangException on empty"
    rationale: "Consistent with ToString() pattern for zero-arg methods; FunLangException is catchable by try-with"
...
Added `HashSet` and `Queue` as first-class mutable collection types to FunLang. `HashSet()` creates an empty unique-element set, `.Add(v)` returns bool (true if new element, false if duplicate), `.Contains(v)` tests membership, and `.Count` returns the element count. `Queue()` creates an empty FIFO queue, `.Enqueue(v)` adds to the back (returns unit), `.Dequeue()` removes and returns the front element (raises a catchable exception on empty queue), and `.Count` returns element count. The type checker synthesizes `TData("HashSet",[])` and `TData("Queue",[])` for constructors and provides typed field access rules for all methods.

## Tasks Completed

| Task | Name | Commit | Files |
|------|------|--------|-------|
| 1 | Add HashSetValue and QueueValue DU cases to Ast.fs | 9527871 | src/FunLang/Ast.fs, src/FunLang/Eval.fs |
| 2 | Constructor interception and FieldAccess dispatch in Eval.fs and Bidir.fs | 90f5081 | src/FunLang/Eval.fs, src/FunLang/Bidir.fs |
...
- Empty dequeue: raises FunLangException with message "Queue.Dequeue: queue is empty", catchable via try-with
...
| Empty dequeue error | FunLangException(StringValue msg) | Catchable by user code via try-with |
