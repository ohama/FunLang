module List =
    ...
    let rec fold (f : 'b -> 'a -> 'b) (acc : 'b) (xs : 'a list) : 'b = match xs with | [] -> acc | h :: t -> fold f (f acc h) t
    ...
    let rec init (n : int) (f : int -> 'a) : 'a list =
        if n = 0 then []
            ...
            let rec _init_helper (i : int) : 'a list =
                if i = n then []
    ...
    let partition (pred : 'a -> bool) (xs : 'a list) : ('a list * 'a list) =
        let rec go (yes : 'a list) (no : 'a list) (xs : 'a list) : ('a list * 'a list) =
            match xs with
    ...
    let rec scan (f : 'b -> 'a -> 'b) (acc : 'b) (xs : 'a list) : 'b list =
        acc :: (match xs with
               | [] -> []
    ...
    let rec pairwise (xs : 'a list) : ('a * 'a) list =
        ...
        | _ -> []
    let sumBy (f : 'a -> int) (xs : 'a list) : int = fold (fun acc x -> acc + f x) 0 xs
    let sum (xs : int list) : int = fold (fun acc x -> acc + x) 0 xs
    let minBy (f : 'a -> 'b) (xs : 'a list) : 'a =
        match xs with
        | [] -> failwith "List.minBy: empty list"
        | h :: t -> fold (fun best x -> if f x < f best then x else best) h t
    ...
    let iter (f : 'a -> unit) (xs : 'a list) : unit = fold (fun _u x -> f x) () xs
